...
calculate.phia_survey.art_coverage <- function(phia_data, age_min_inclusive, age_max_inclusive, regions=NULL ) {
  phia_data = phia_data %>%
    filter(age >= age_min_inclusive,
           age <= age_max_inclusive,
           hivstatusfinal == 1,
  ...
  get_ci_from_design = function(design) {
    ci = svyciprop(~I(art==1),design, method = "mean",df=25)
    data.frame(art_coverage=ci[[1]],
               lb=attributes(ci)$ci[["2.5%"]],
               ub=attributes(ci)$ci[["97.5%"]])
  }
...
calculate.phia_survey.effective_count = function(input, target_variable='prevalence') {
  ...
  for(row_iter in seq(1,nrow(input))){
    ...
    to_minimize <- function(alpha){
      alpha <- abs(alpha)
      beta_computed <- -alpha + ( (alpha)/input[row_iter, target_variable] ) #alpha*((1-input[row_iter,'prevalence'])/input[row_iter,'prevalence'])
    ...
    beta_optimized[row_iter]  <- -alpha_optimized[row_iter]  + ( (alpha_optimized[row_iter])/input[row_iter, target_variable] ) #-alpha + ( (alpha)/input[row_iter,'prevalence'] )
    ...
    print(paste("Was aiming for mean of ", input[row_iter,target_variable], "and got",   mean_fitted[row_iter] ))
  ...
  input$pct_diff_mean   <- (input$mean_fitted-input[,target_variable])/input$mean_fitted
